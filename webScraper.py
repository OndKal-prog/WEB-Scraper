...
def scrape(link, nazev_vystup_souboru):
    print("DEBUG: Funkce scrape byla spuštěna.")
    parsed_odkaz = urlparse(link)
    ...
    try:
        print(f"DEBUG: Pokouším se stáhnout hlavní URL: {link}")
        page_response = requests.get(link, timeout=10)
        page_response.raise_for_status()
        print("DEBUG: Hlavní URL úspěšně stažena.")
    except requests.exceptions.RequestException as e:
        print(f"Chyba při stahování hlavní URL '{link}': {e}")
        sys.exit(1)
    ...
    try:
        with open(nazev_vystup_souboru, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        print(f"\nData byla úspěšně uložena do souboru '{nazev_vystup_souboru}'")
