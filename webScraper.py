...
def scrape(odkaz, nazev_vystupniho_souboru):
    ...
    for cislo_obce_element in hledam_cislo_obce:
        cislo_obci.append(cislo_obce_element.text.strip())
    for td_element in hledam_jmeno_obce:
        nazev_obci.append(td_element.text.strip())

    print(f"Nalezeno {len(cislo_obci)} obcí ke zpracování.")
    ...
    for aktualni_cislo_obce in cislo_obci:
        ...
        try:
            ...
            pocet_volicu.append(volice.text.replace('\xa0', ' ').strip() if volice else "N/A")

            obalky = soup1.find("td", class_="cislo", headers="sa5")
            pocet_obalek.append(obalky.text.replace('\xa0', ' ').strip() if obalky else "N/A")

            valid_obalky = soup1.find("td", class_="cislo", headers="sa6")
            pocet_valid_obalek.append(valid_obalky.text.replace('\xa0', ' ').strip() if valid_obalky else "N/A")
    ...
    try:
        with open(nazev_vystupniho_souboru, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for i in range(pocet_obci_k_zapisu):
                row_data = {
                    "Cislo Obce": cislo_obci[i],
                    "Nazev Obce": nazev_obci[i],
                    "Pocet Volicu": pocet_volicu[i],
                    "Pocet Obalek": pocet_obalek[i],
                    "Pocet Validnich Obalek": pocet_valid_obalek[i]
                }
